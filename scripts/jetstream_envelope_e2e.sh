...
#   JETSTREAM_DIAG_STACK="jsr_client"   # router_nats | jsr_client
...
JETSTREAM_DIAG_STACK="${JETSTREAM_DIAG_STACK:-jsr_client}"
...
echo "Starting JetStream envelope diag server: NATS_URL=$NATS_URL RUN_ID=$JETSTREAM_DIAG_RUN_ID STACK=$JETSTREAM_DIAG_STACK SUBJECT=$JETSTREAM_DIAG_SUBJECT QUEUE=$JETSTREAM_DIAG_QUEUE SID=$JETSTREAM_DIAG_SID FAIL_FIRST_N=$JETSTREAM_DIAG_FAIL_FIRST_N"
(
  cd "$ROOT_DIR"
