...
#    - deployment history contains vendor sync entry for target hive
#
# Usage:
#   BASE="http://127.0.0.1:8080" HIVE_ID="worker-220" \
#     bash scripts/orchestrator_drift_vendor_e2e.sh
#
# Optional:
...
for item in entries:
    ...
    if kind in ("pre_sync_mismatch_resolved", "post_sync_mismatch", "sync_error", "health_check_failed") and severity in ("warning", "error"):
        print("1")
        raise SystemExit(0)
...
if [[ -z "$baseline_remote_hash" ]]; then
  echo "FAIL: remote vendor hash is empty before tamper (missing ${REMOTE_VENDOR_BIN}?)" >&2
  exit 1
fi
...
if ! remote_syncthing_service_active; then
  echo "FAIL: ${REMOTE_SYNCTHING_SERVICE} is not active before drift injection" >&2
  exit 1
fi
...
while true; do
  ...
  if (( elapsed > WAIT_TIMEOUT_SECS )); then
    echo "FAIL: timeout waiting vendor drift evidence (${WAIT_TIMEOUT_SECS}s)" >&2
    break
  fi
  ...
  status_alerts="$(http_call "GET" "$BASE/drift-alerts?hive=$HIVE_ID&category=vendor&limit=50" "$alerts_body")"
  log_http_response "$status_alerts" "$alerts_body"
  alerts_ok=0
  if [[ "$status_alerts" == "200" && "$(has_recent_vendor_drift_alert "$alerts_body" "$drift_start_ms")" == "1" ]]; then
    alerts_ok=1
  fi
  ...
  deploy_ok=0
  if [[ "$status_deploy" == "200" && "$(has_recent_vendor_deployment "$deployments_body" "$drift_start_ms")" == "1" ]]; then
    deploy_ok=1
  fi
  ...
  if [[ "$current_remote_hash" == "$baseline_local_hash" && "$alerts_ok" == "1" && "$deploy_ok" == "1" && "$health_ok" == "1" ]]; then
    echo "OK: vendor drift reconciled and evidenced via API"
    echo "vendor drift E2E passed."
    exit 0
  fi

  echo "[WAIT] elapsed=${elapsed}s remote_hash=${current_remote_hash:-<none>} alerts_ok=${alerts_ok} deploy_ok=${deploy_ok} health_ok=${health_ok}" >&2
