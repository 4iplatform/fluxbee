...
usage() {
  cat <<'EOF'
Usage:
  publish-io-linkedhelper-runtime.sh --version <version> [options]

Required:
  --version <version>      Runtime version (example: 0.1.0)

Options:
  --runtime <name>         Override runtime key (default: io.linkedhelper)
  --binary <path>          Override binary path (default: nodes/io/target/release/io-linkedhelper)
...
if [[ "$SKIP_BUILD" != "1" ]]; then
  ...
  (cd "$ROOT_DIR" && cargo build --release --manifest-path nodes/io/Cargo.toml -p io-linkedhelper)
fi
...
DEFAULT_RUST_LOG="info,io_linkedhelper=debug,io_common=debug,fluxbee_sdk=info"
...
echo "Configured io.linkedhelper start.sh default RUST_LOG: $DEFAULT_RUST_LOG"
