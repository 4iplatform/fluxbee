# Fluxbee — Master Rearchitecture Spec v3

**Status:** canonical rearchitecture proposal — contract-frozen implementation version  
**Date:** 2026-04-23  
...
### 4.1 `desired_state`
Contains only information needed by the reconciler:
...
- nodes
- routing
- policy (when supported by snapshot/reconciler scope)
- identity (when supported by snapshot/reconciler scope)
...
- runtime base references are coherent
...
To keep the first reconciler tractable, snapshot v1 covers only:

- hive reachability / liveness
- runtimes
- nodes
- routes
- VPNs if topology reconciliation is enabled in v1
...
Those remain out of reconciler scope until a later contract freeze.
...
- `/hives/{hive}/vpns` if VPN reconciliation is in scope
...
- `NODE_RESTART_ONLY`
- `NODE_KILL`
- `NODE_RECREATE` (destructive, explicit only)
