...
- [x] GO-SDK-7. ~~Revisar si conviene extender `ANNOUNCE` con `shm_name` explícito para evitar lectura adicional de `identity.yaml`.~~
  - **CANCELADO y limpiado.** Esta tarea era una optimización de `PeerIdentityResolver`. Con L2-LOOKUP completo, ese resolver quedó sin callers en todo el repo. Se eliminaron `identity.go`, `identity_linux.go`, `identity_other.go` e `identity_test.go` — ~290 líneas de código muerto.
...
- [x] GO-SDK-9. ~~Portar readers SHM adicionales que hoy siguen faltando respecto del runtime Rust.~~
  - **CANCELADO — objetivo logrado por otro camino.** La paridad con Rust se logró con el stamp `routing.src_l2_name` del router, no con readers SHM adicionales. El único reader SHM que existía en Go fue eliminado junto con GO-SDK-7.
