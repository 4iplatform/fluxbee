...
- [x] T4.1 Documentar explícitamente que `PUT .../config` no reemplaza `CONFIG_SET` runtime node-owned.
- [x] T4.2 Definir estrategia para `CONFIG_CHANGED` en IO:
  - opción A: solo señal informativa,
  - opción B: puente interno a `CONFIG_SET` en runtime IO,
  - elegir una y normarla.
- [ ] T4.3 Alinear UX de SY.admin/SY.architect para recomendar `control/config-set` como camino de hot reload IO. (pendiente externo / sugerencia a core, no bloqueante para IO)

Nota avance T4.1:
- docs IO actualizados con frontera explícita:
  - `PUT .../config` = capa core/orchestrator (`config.json`)
  - `CONFIG_SET` = capa runtime node-owned (hot reload IO)

Nota avance T4.2:
- estrategia elegida para IO v1: opción A (`CONFIG_CHANGED` informativo).
- operación runtime/business canónica en IO: `CONFIG_SET`.
- `io-slack` ahora consume `CONFIG_CHANGED` como señal informativa (log + no apply runtime).

Sugerencia UX para core (no bloqueante):
- En SY.admin/SY.architect, recomendar explícitamente `POST .../control/config-set` para hot reload IO.
- Mantener `PUT .../config` como vía de persistencia core/orchestrator (`config.json`), sin asumir apply runtime inmediato.
- Esta mejora es de UX operativa; no es requisito funcional para cerrar la implementación IO.

## Fase 5 - Seguridad, observabilidad y operación

- [x] T5.1 Redaction consistente de secretos en:
  - `CONFIG_RESPONSE`,
  - `STATUS`,
  - logs.
- [x] T5.2 Métricas mínimas del control-plane IO:
  - count de `CONFIG_SET ok/error`,
  - último `config_version`,
  - estado actual.
- [x] T5.3 Runbook operativo por adapter:
  - bootstrap mínimo,
  - secuencia `CONFIG_GET -> CONFIG_SET`,
  - recuperación desde `FAILED_CONFIG`.

Nota operativa acordada para Fase 5 (logging):
- política de logs: event-driven (por evento relevante), sin logging periódico de heartbeat/config.
- objetivo: mensajes legibles por humanos + campos estructurados mínimos para debug (`node_name`, `config_version`, `error_code`, `trace_id` cuando aplique).
- implementación base reusable movida a `io-common` (`io_control_plane_logging`), para reutilizar en adapters IO futuros.
- `io-sim` consume los mismos helpers para `CONFIG_RESPONSE`, manteniendo formato uniforme entre adapters.

Nota avance T5.2:
- métrica common reusable en `io-common` (`io_control_plane_metrics`).
- `io-slack` registra `CONFIG_SET ok/error` y expone snapshot en `CONFIG_GET` bajo `metrics.control_plane`.
- snapshot incluye: `config_set_ok`, `config_set_error`, `last_config_version`, `current_state`, `last_error_code` (opcional).

Nota avance T5.1:
- `CONFIG_RESPONSE` mantiene `effective_config` redacted vía contrato de adapter.
- `STATUS` implementado en `io-slack` como respuesta segura (`STATUS_RESPONSE`) sin exponer secretos.
- logging de control-plane consolidado en `io-common` con política event-driven (sin logs periódicos).

Nota avance T5.3:
- `docs/io/io-slack-deploy-runbook.md` actualizado al estado operativo actual:
  - contrato de spawn estricto (managed, sin fallback legacy),
  - hot reload runtime vía `CONFIG_GET -> CONFIG_SET`,
  - sección explícita de recuperación desde `FAILED_CONFIG`.

## Fase 6 - Tests y criterios de aceptación

- [x] T6.1 Unit tests de control-plane común IO.
- [x] T6.2 Tests de precedencia (persistida > orchestrator fallback > unconfigured).
- [x] T6.3 Tests de versionado (`stale_config_version`, idempotencia).
...
Nota avance T6.1/T6.2/T6.3:
- `io-common` ya tenía base de tests para parse/validación de control-plane y bootstrap de precedencia.
- se agregó helper común `ensure_config_version_advances(...)` en `io_control_plane` con tests unitarios para:
  - versión mayor (`ok`),
  - replay idempotente (`incoming == current` => `stale_config_version`),
  - versión menor (`incoming < current` => `stale_config_version`).
- `io-slack` usa ese helper común para unificar la validación de `config_version` en `CONFIG_SET`.
- guía de ejecución manual para `T6.4`: `docs/onworking NOE/t6_4_io_slack_e2e_steps.md`.
