...
- If a hive-specific endpoint returns an empty list, report that it returned no entries. Do not invent filtered rows from broader results.
...
impl FunctionTool for ArchitectSystemGetTool {
    fn definition(&self) -> FunctionToolDefinition {
        FunctionToolDefinition {
            ...
            description: format!(
                "Read live Fluxbee system state through SY.admin over socket for hive {hive}. Read-only. Supports GET paths and safe POST checks such as OPA policy validation or non-SY node CONFIG_GET control-plane discovery. Use /admin/actions or /admin/actions/{{action}} when you need dynamic help; those responses include standardized request_contract metadata, body fields, notes, and example_scmd values. Distinguish runtime names from node instances: `AI.chat` is a runtime, while `AI.chat@{hive}` is a node instance. Use /inventory or /inventory/summary for system-wide node visibility, use /versions or /hives/{hive}/versions for core and runtime versions, use /hives/{hive}/runtimes or /hives/{hive}/runtimes/AI.chat for runtime/package info, and use /hives/{hive}/deployments or /hives/{hive}/drift-alerts for hive-scoped views instead of locally filtering global lists. If a hive endpoint returns zero entries, report zero entries instead of inventing filtered results. Example paths: /inventory, /inventory/summary, /inventory/{hive}, /versions, /hives/{hive}/versions, /hives/{hive}/runtimes, /hives/{hive}/runtimes/AI.chat, /hives/{hive}/deployments, /hives/{hive}/drift-alerts, /hives/{hive}/nodes, /hives/{hive}/nodes/SY.admin@{hive}/status, /hives/{hive}/nodes/AI.chat@{hive}/control/config-get, /hives/{hive}/identity/ilks, /admin/actions, /admin/actions/get_node_status, /admin/actions/get_versions, /config/storage",
