...
fn resolve_add_hive_ingress_section(
    ...
    let ing = payload.get("ingress").and_then(|v| v.as_object()).ok_or(
        "add_hive role=ingress requires an 'ingress' object (listen, optional endpoints_json)",
    )?;
    let get_str = |key: &str| {
        ing.get(key)
            .and_then(|v| v.as_str())
    ...
    let listen = get_str("listen").ok_or(
        "add_hive role=ingress requires 'ingress.listen' (host:port for the public frontend)",
    )?;
    let listen_addr: std::net::SocketAddr = listen.parse().map_err(|_| {
        format!("invalid ingress.listen '{listen}' (want host:port, e.g. 0.0.0.0:443)")
    })?;
    ...
    let allow_plaintext = match ing.get("allow_plaintext") {
        Some(value) => value
            .as_bool()
            .ok_or("ingress.allow_plaintext must be boolean")?,
        None => false,
    };
    ...
    if tls_vault_key.is_none() {
        if !allow_plaintext {
            return Err(
                "add_hive role=ingress requires ingress.tls_vault_key; set ingress.allow_plaintext=true only for explicit dev plaintext"
                    .into(),
            );
        }
        if listen_addr.port() == 443 {
            return Err("ingress.allow_plaintext=true is not allowed on port 443".into());
        }
    }
    if let Some(seed) = ing.get("endpoints_json") {
        if !ingress_endpoints_seed_is_empty(seed) {
            return Err(
                "ingress.endpoints_json is no longer accepted; publish URLs with externalize"
                    .into(),
            );
        }
    }
...
fn ingress_endpoints_seed_is_empty(value: &serde_json::Value) -> bool {
    value
        .get("endpoints")
...
async fn main() -> Result<(), OrchestratorError> {
    ...
    let role = HiveRole::from_role(hive.role.as_deref()).expect("role validated by the gate above");
...
fn ensure_mesh_ca() -> Result<json_router::mesh_tls::MeshCa, OrchestratorError> {
    ...
    let load = || {
        MeshCa::from_pem(&fs::read_to_string(&ca_crt)?, &fs::read_to_string(&ca_key)?)
...
fn ensure_motherbee_ssh_key() {
        ...
        .output()
    {
        Ok(o) if o.status.success() => {
            ...
            tracing::info!(
                path = MOTHERBEE_SSH_KEY_PATH,
                "generated motherbee SSH bootstrap key"
            );
        }
        Ok(o) => tracing::warn!(
        ...
        Err(err) => {
            tracing::warn!(error = %err, "ssh-keygen unavailable; motherbee SSH key not generated")
        }
    }
}
...
async fn handle_system_update_message(
    ...
    let runtime_health_context = if category == "runtime" {
        load_runtime_manifest_result()
            .ok()
            .flatten()
            .map(|manifest| {
                let targeted = runtime_health_snapshot(
                    &manifest,
                    Path::new(DIST_RUNTIME_ROOT_DIR),
                    request.runtime.as_deref(),
                    request.runtime_version.as_deref(),
                ...
                .unwrap_or_else(|err| {
                    serde_json::json!({
                        "scope": if request.runtime.is_some() { "targeted" } else { "global_current" },
                        "status": "error",
                        "message": err.to_string(),
                    })
                });
                let global = runtime_health_snapshot(
                    &manifest,
                    Path::new(DIST_RUNTIME_ROOT_DIR),
                    None,
                    None,
    ...
    match apply_system_update_local(state, &request).await {
        Ok(result) => serde_json::json!({
            ...
            "runtime": request.runtime.clone(),
            "runtime_version": request.runtime_version.clone(),
            "readiness_scope": if request.runtime.is_some() { "targeted" } else { "global" },
            "targeted_runtime_health": runtime_health_context
                .as_ref()
                .map(|(targeted, _)| targeted.clone())
                .unwrap_or(serde_json::Value::Null),
            "global_runtime_health": runtime_health_context
                .as_ref()
...
fn render_worker_egress_yaml(egress: Option<&EgressSection>) -> Result<String, OrchestratorError> {
    let Some(egress) = egress else {
        return Ok(String::new());
    };
    let Some(gateway_ip) = egress
        .gateway_ip
        .as_deref()
        .map(str::trim)
    ...
    if egress.ipv6.trim() != "blocked" {
        return Err(format!(
            "invalid egress.ipv6 '{}': only \"blocked\" is supported for worker route injection",
            egress.ipv6
        )
        .into());
    }
    let gateway_ip: std::net::Ipv4Addr = gateway_ip
        .parse()
...
fn first_usable_ipv4_from_network(
    cidr: &str,
    network: u32,
    prefix: u32,
) -> Result<std::net::Ipv4Addr, OrchestratorError> {
    if prefix >= 31 {
        return Err(format!(
            "invalid lan_cidr '{cidr}': prefix /{prefix} has no derived gateway address"
        )
        .into());
    }
    Ok(std::net::Ipv4Addr::from(network + 1))
}

fn parse_ipv4_cidr_network(cidr: &str) -> Result<(u32, u32, u32), OrchestratorError> {
    let (ip, pfx) = cidr
        .split_once('/')
...
fn require_field<'a>(value: Option<&'a String>, field: &str) -> Result<&'a str, OrchestratorError> {
    value
        .map(|s| s.trim())
        .filter(|s| !s.is_empty())
        .ok_or_else(|| {
...
fn resolve_egress_nat_config(eg: &EgressSection) -> Result<EgressNatConfig, OrchestratorError> {
        ...
        .filter(|s| !s.is_empty())
    {
        ...
        None => first_usable_ipv4_from_network(&lan_cidr, lan_network, lan_prefix)?,
    };
    if u32::from(edge_ip) & lan_mask != lan_network {
        return Err(format!(
            "invalid egress.edge_ip '{edge_ip}': not within egress.lan_cidr '{lan_cidr}'"
        )
        .into());
    }
    Ok(EgressNatConfig {
...
/// Open Question #3). Reconcile deletes an existing Fluxbee-owned table before
/// applying this definition, so re-apply is idempotent and never duplicates
/// rules.
...
fn reconcile_egress_nat(eg: &EgressSection) -> Result<EgressVerification, OrchestratorError> {
    ...
    verification.ipv6_blocked = read_sysctl_bool("net/ipv6/conf/all/disable_ipv6") == Some(true);

    // conntrack tuning: live max + persisted file + modprobe hashsize (boot).
    write_file_if_changed(
        EGRESS_CONNTRACK_SYSCTL_PATH,
        &egress_conntrack_sysctl_content(),
    )?;
    write_file_if_changed(
...
fn validate_blob_runtime_for_remote_hive(
    blob: &BlobRuntimeConfig,
) -> Result<(), OrchestratorError> {
...
fn validate_dist_runtime_for_remote_hive(
    dist: &DistRuntimeConfig,
) -> Result<(), OrchestratorError> {
...
fn runtime_api_entry(
    ...
    serde_json::json!({
        ...
        "materialization_summary": runtime_materialization_summary_for_entry(
            runtime,
            entry,
            runtime_entries,
        ),
...
async fn local_runtime_snapshot(
    ...
    match load_runtime_manifest_result() {
        Ok(Some(manifest)) => {
            ...
            let targeted_runtime_health = runtime_health_snapshot(
                &manifest,
                Path::new(DIST_RUNTIME_ROOT_DIR),
                Some(runtime),
                version_filter,
            ...
            let global_runtime_health =
                runtime_health_snapshot(&manifest, Path::new(DIST_RUNTIME_ROOT_DIR), None, None)
                    .unwrap_or_else(|err| {
                        serde_json::json!({
                            "scope": "global_current",
                            "status": "error",
            ...
            serde_json::json!({
                ...
                "materialization": runtime_materialization_for_entry(
                    runtime,
                    entry,
                    runtime_entries_snapshot.as_ref(),
                    version_filter,
...
fn remove_hive_cleanup_local_flow(egress_teardown: bool) -> serde_json::Value {
    let deferred_script = format!(
        "sleep 1; {}",
        remove_hive_cleanup_full_script(egress_teardown)
    );
...
fn runtime_spawn_preflight_with_manifest(
    manifest: &RuntimeManifest,
    requested_runtime: &str,
    requested_version: &str,
    ...
    runtime_spawn_preflight_with_manifest_and_root(
        manifest,
        requested_runtime,
        requested_version,
        entrypoint,
...
fn runtime_spawn_preflight_with_manifest_and_root(
    manifest: &RuntimeManifest,
    requested_runtime: &str,
    requested_version: &str,
    entrypoint: &RuntimeSpawnEntrypoint,
    ...
    let runtime_entries = manifest.runtimes.as_object();
    let runtime_map = runtime_manifest_entry_map(manifest).map_err(|err| {
        serde_json::json!({
            "status": "error",
            "error_code": "MANIFEST_INVALID",
    ...
    let requested_entry = runtime_map.get(requested_runtime).ok_or_else(|| {
        serde_json::json!({
            "status": "error",
            "error_code": "RUNTIME_NOT_AVAILABLE",
            "message": format!("runtime '{}' not found in manifest", requested_runtime),
            "runtime": requested_runtime,
            "version": requested_version,
            "target": target_hive,
            "node_name": node_name,
    ...
    let requested_materialization = runtime_materialization_for_entry_with_root(
        requested_runtime,
        requested_entry,
        runtime_entries,
        runtimes_root,
    ...
    let global_runtime_health = runtime_health_snapshot(manifest, runtimes_root, None, None)
        .unwrap_or_else(|err| {
            serde_json::json!({
                "scope": "global_current",
                "status": "error",
    ...
    if let Some(runtime_base) = entrypoint.runtime_base.as_deref() {
        ...
        .map_err(|mut payload| {
            if let Some(obj) = payload.as_object_mut() {
                obj.insert(
                    "requested_runtime_materialization".to_string(),
                    requested_materialization,
                );
                obj.insert(
                    "runtime_base_materialization".to_string(),
                    base_materialization,
                );
                obj.insert(
                    "targeted_runtime_health".to_string(),
        ...
        .map_err(|mut payload| {
            if let Some(obj) = payload.as_object_mut() {
                obj.insert(
                    "runtime_materialization".to_string(),
                    requested_materialization,
                );
                obj.insert(
                    "targeted_runtime_health".to_string(),
...
fn runtime_health_snapshot(
    ...
    let errors = verify_runtime_artifacts_for_scope(
        manifest,
        runtimes_root,
        runtime_filter,
        runtime_version_filter,
    )?;
    Ok(serde_json::json!({
        "scope": if runtime_filter.is_some() { "targeted" } else { "global_current" },
        "runtime": runtime_filter,
        "runtime_version": runtime_version_filter,
...
fn runtime_materialization_for_entry(
    runtime: &str,
    entry: &RuntimeManifestEntry,
    runtime_entries: Option<&serde_json::Map<String, serde_json::Value>>,
    version_filter: Option<&str>,
) -> serde_json::Value {
    runtime_materialization_for_entry_with_root(
        runtime,
        entry,
        runtime_entries,
        Path::new(DIST_RUNTIME_ROOT_DIR),
...
fn runtime_materialization_for_entry_with_root(
    runtime: &str,
    entry: &RuntimeManifestEntry,
    runtime_entries: Option<&serde_json::Map<String, serde_json::Value>>,
    runtimes_root: &Path,
    ...
    let selected_version = version_filter
        .and_then(|value| {
            let trimmed = value.trim();
            if trimmed.is_empty() || trimmed == "current" {
                entry.current.as_deref().map(str::trim)
            } else {
                Some(trimmed)
            }
        })
    ...
    for version in runtime_versions_from_manifest_entry(entry) {
        ...
        let package_dir = runtimes_root.join(runtime).join(&version);
        let package_dir_present = package_dir.is_dir();
        let start_script_path = package_dir.join("bin/start.sh");

        let mut detail = serde_json::Map::new();
        detail.insert(
            "package_dir".to_string(),
            serde_json::json!(package_dir.display().to_string()),
        );
        detail.insert(
            "package_dir_present".to_string(),
            serde_json::json!(package_dir_present),
        );

        match package_type {
            "full_runtime" => {
                let start_present = start_script_path.is_file();
                let start_exec =
                    start_present && local_runtime_script_is_executable(&start_script_path);
                let blocking_reason = if !package_dir_present {
                    Some("missing_package_dir")
                } else if !start_present {
                    Some("missing_start_script")
                } else if !start_exec {
                    Some("start_script_not_executable")
                } else {
                    None
                };
                detail.insert(
                    "start_script_path".to_string(),
                    serde_json::json!(start_script_path.display().to_string()),
                );
                detail.insert(
                    "start_script_present".to_string(),
                    serde_json::json!(start_present),
                );
                detail.insert(
                    "start_script_executable".to_string(),
                    serde_json::json!(start_exec),
                );
                detail.insert(
                    "materialized".to_string(),
                    serde_json::json!(blocking_reason.is_none()),
                );
                detail.insert(
                    "blocking_reason".to_string(),
                    blocking_reason.map_or(serde_json::Value::Null, serde_json::Value::from),
                );
            }
            "config_only" | "workflow" => {
                ...
                let blocking_reason = if !package_dir_present {
                    Some("missing_package_dir")
                } else if base_runtime_name.is_none() {
                    Some("missing_runtime_base")
                } else if base_version.is_none() {
                    Some("missing_runtime_base_current")
                } else if !base_start_present {
                    Some("missing_base_start_script")
                } else if !base_start_exec {
                    Some("base_start_script_not_executable")
                } else {
                    None
                };
                detail.insert(
                    "runtime_base".to_string(),
                    base_runtime_name.map_or(serde_json::Value::Null, serde_json::Value::from),
                );
                detail.insert(
                    "runtime_base_version".to_string(),
                    base_version.map_or(serde_json::Value::Null, serde_json::Value::from),
                );
                detail.insert(
                    "base_start_script_path".to_string(),
                    base_start.as_ref().map_or(serde_json::Value::Null, |path| {
                        serde_json::json!(path.display().to_string())
                    }),
                );
                detail.insert(
                    "base_start_script_present".to_string(),
                    serde_json::json!(base_start_present),
                );
                detail.insert(
                    "base_start_script_executable".to_string(),
                    serde_json::json!(base_start_exec),
                );
                detail.insert(
                    "materialized".to_string(),
                    serde_json::json!(blocking_reason.is_none()),
            ...
            other => {
                detail.insert("materialized".to_string(), serde_json::json!(false));
                detail.insert(
                    "blocking_reason".to_string(),
                    serde_json::json!(format!("unknown_package_type:{other}")),
                );
            }
        }

        versions.insert(version, serde_json::Value::Object(detail));
    }
...
fn runtime_materialization_summary_for_entry(
    runtime: &str,
    entry: &RuntimeManifestEntry,
    runtime_entries: Option<&serde_json::Map<String, serde_json::Value>>,
) -> serde_json::Value {
...
fn runtime_materialization_summary_for_entry_with_root(
    ...
    let versions = detail
        ...
        .cloned()
        .unwrap_or_default();
    let materialized_versions = versions
        .values()
        .filter(|value| {
            value
                .get("materialized")
    ...
    let current_version = entry
        ...
        .map(str::trim)
        .filter(|value| !value.is_empty());
    let current_detail = current_version.and_then(|version| versions.get(version));
    serde_json::json!({
        "known_versions": versions.len(),
        "materialized_versions": materialized_versions,
        "current_version": current_version,
        "current_materialized": current_detail
            .and_then(|value| value.get("materialized"))
            .and_then(|value| value.as_bool())
            .unwrap_or(false),
        "current_blocking_reason": current_detail
...
async fn rollback_node_identity_after_failed_spawn(
    ...
    if created_remote_ilk {
        if let Some(ilk_id) = identity_ilk_id {
            ...
            let delete =
                delete_node_ilk_for_spawn_rollback(state, identity_primary_hive_id, ilk_id).await;
            rollback["remote_ilk_deleted"] = serde_json::json!(delete
                .get("status")
                .and_then(|value| value.as_str())
                .is_some_and(|status| status == "ok"));
            if delete.get("status").and_then(|value| value.as_str()) != Some("ok") {
                rollback["status"] = serde_json::json!("error");
            }
...
async fn run_node_flow(
    ...
    if let Err(payload) = runtime_spawn_preflight_with_manifest(
        &manifest,
        &runtime_key,
        &version,
        &entrypoint,
...
fn sync_vendor_to_worker(
    ...
    let Some(manifest_path) = local_vendor_manifest_path() else {
        tracing::warn!(
            hive_id = hive_id,
            "no local vendor manifest; skipping vendor sync to worker"
        );
        return Ok(());
    };
    ...
    if !bin_path.exists() {
        tracing::warn!(
            hive_id = hive_id,
            "vendor manifest present but syncthing binary missing; skipping vendor sync"
        );
        return Ok(());
    }
    let cfg_path = local_vendor_component_path("syncthing/config.xml");

    let mut upload_paths = vec![
    ...
    tracing::info!(
        hive_id = hive_id,
        "synced vendor runtime (syncthing) to worker"
    );
    Ok(())
}
...
fn remote_wait_service_active(
    ...
    let check_running = || -> Result<bool, OrchestratorError> {
        ssh_with_key(address, key_path, &sudo_wrap(&is_active_cmd), user)?;
        let substate = ssh_with_key_output(address, key_path, &sudo_wrap(&substate_cmd), user)
            .unwrap_or_default();
    ...
    for _ in 0..timeout_secs {
        match check_running() {
            ...
            Err(err) => {
                ...
                let summary =
                    ssh_with_key_output(address, key_path, &sudo_wrap(&state_summary_cmd), user)
                        .unwrap_or_default();
...
fn restart_remote_core_services_with_health_gate(
    ...
    for service in services {
        ...
        ssh_with_key(address, key_path, &sudo_wrap(&restart_cmd), user)
...
async fn add_hive_flow(
        ...
        if let Err(err) = probe_remote_orchestrator_socket_ready(state, hive_id, address).await {
            ...
            .await
            {
                ...
                Err(err) => {
                    ...
                    let (error_code, retryable, message) = if is_socket_only_unreachable_error(
                        &err_text,
                    ) {
                        (
    ...
    let remote_core_hash_before =
        remote_core_manifest_hash(address, &key_path, creds.user.as_str())
            .ok()
            .flatten();
    ...
    let remote_core_hash_after = remote_core_manifest_hash(address, &key_path, creds.user.as_str())
        .ok()
        .flatten();
    let entry = DeploymentHistoryEntry {
        deployment_id: Uuid::new_v4().to_string(),
    ...
    let worker_egress_yaml = match render_worker_egress_yaml(state.egress.as_ref()) {
        Ok(value) => value,
        Err(err) => {
            return serde_json::json!({
                "status": "error",
                "error_code": "CONFIG_FAILED",
                "message": format!("invalid motherbee egress config: {err}"),
            });
        }
    };
    ...
    if let Err(err) = remote_wait_service_active(
        ...
        let journal_tail = remote_service_journal_tail(
            address,
            &key_path,
            creds.user.as_str(),
            "sy-orchestrator",
...
async fn add_egress_hive_flow(
    ...
    let worker_uplink = match resolve_worker_uplink_address(&wan_listen, address) {
        ...
        Err(err) => {
            return err_payload(
                "CONFIG_FAILED",
                format!("resolve egress uplink failed: {err}"),
            )
        }
    };
    let storage_path = state
    ...
    if let Err(err) = remote_wait_service_active(
        ...
        let journal = remote_service_journal_tail(
            address,
            &key_path,
            creds.user.as_str(),
            "sy-orchestrator",
...
async fn add_ingress_hive_flow(
    ...
    // The edge is born without public routes. URLs are opened one at a time by
    // SY.admin through EDGE_OPEN_URL after ownership has been resolved.
    let endpoints_json = "{\n  \"endpoints\": []\n}\n".to_string();
    if let Err(err) = write_remote_file(
        address,
        &key_path,
    ...
    let mut services_to_start = vec!["rt-gateway".to_string(), "sy-orchestrator".to_string()];
    for node_name in &ingress_system_nodes.nodes {
        let service = name_to_service(node_name);
        if !services_to_start.iter().any(|name| name == &service) {
    ...
    for node_name in &ingress_system_nodes.wait_for {
        let service = name_to_service(node_name);
        if let Err(err) = remote_wait_service_active(
            address,
            &key_path,
            ...
            let journal =
                remote_service_journal_tail(address, &key_path, creds.user.as_str(), &service, 120);
            return err_payload(
                "SERVICE_FAILED",
                match journal {
                    Some(tail) => format!("{service} failed health gate: {err}; journal={tail}"),
                    None => format!("{service} failed health gate: {err}"),
                },
            );
        }
    }
        ...
        .any(|name| name.trim() == "SY.edge")
    {
        if let Err(err) = remote_wait_service_active(
            ...
            return err_payload(
                "SERVICE_FAILED",
                match journal {
                    Some(tail) => format!("sy-edge failed health gate: {err}; journal={tail}"),
                    None => format!("sy-edge failed health gate: {err}"),
                },
            );
        }
        edge_service_active = true;
    }
...
fn resolve_add_hive_egress_section(
    ...
    let eg = payload.get("egress").and_then(|v| v.as_object()).ok_or(
        "add_hive role=egress requires an 'egress' object (lan_cidr, wan_iface, lan_iface)",
    )?;
    let get_str = |key: &str| {
        eg.get(key)
            .and_then(|v| v.as_str())
...
fn apply_add_hive_ssh_controls_after_finalize(
    ...
    if restrict_ssh_requested {
        ...
        .and_then(|_| ssh_with_key(address, key_path, "sudo -n /bin/bash -lc 'exit 0'", user));
...
if command -v sshd >/dev/null 2>&1; then
  ...
  /usr/sbin/sshd -t >/dev/null 2>&1 || true
    ...
    ssh_with_key(address, key_path, &sudo_wrap(set_password_auth_cmd), user)?;

    let restart_ssh_cmd = r#"bash -lc "systemctl restart sshd || systemctl restart ssh || service sshd restart || service ssh restart""#;
    ssh_with_key(address, key_path, &sudo_wrap(restart_ssh_cmd), user)?;
    Ok(())
}
    ...
    let ssh_key_removed =
        revoke_bootstrap_authorized_key_with_key(address, &key_path, user, &pub_key).is_ok()
            || password
                .filter(|value| !value.trim().is_empty())
                .is_some_and(|value| {
                    revoke_bootstrap_authorized_key_with_pass(address, user, value, &pub_key)
                        .is_ok()
                });
...
fn ensure_remote_orchestrator_sudoers_with_access(
    ...
    let upload_result = scp_with_key(address, key_path, &local_refs, &remote_tmp, user);
    ...
    ssh_with_key(address, key_path, &sudo_wrap("systemd-run --version"), user).map_err(|err| {
        format!("sudo -n unavailable after sudoers bootstrap (systemd-run): {err}")
    })?;
    ssh_with_key(
        address,
        key_path,
    ...
    ssh_with_key(address, key_path, &sudo_wrap("/bin/chmod --version"), user)
...
mod tests {
    ...
    fn is_socket_only_unreachable_error_matches_real_transport_strings() {
        ...
        .to_string();
        assert!(
            is_socket_only_unreachable_error(&unreachable),
            "{unreachable}"
        );
    ...
    fn first_usable_ipv4_handles_any_mask() {
        ...
        assert!(first_usable_ipv4("192.168.8.0/31").is_err());
        assert!(first_usable_ipv4("192.168.8.1/32").is_err());
    ...
    fn resolve_egress_nat_config_requires_fields_and_blocks_ipv6() {
        ...
        let mut invalid_cidr_with_explicit_edge_ip = egress_section(true);
        invalid_cidr_with_explicit_edge_ip.lan_cidr = Some("192.168.8.0/33".to_string());
        invalid_cidr_with_explicit_edge_ip.edge_ip = Some("192.168.8.1".to_string());
        assert!(resolve_egress_nat_config(&invalid_cidr_with_explicit_edge_ip).is_err());

        let mut explicit_edge_ip_on_single_host_cidr = egress_section(true);
        explicit_edge_ip_on_single_host_cidr.lan_cidr = Some("192.168.8.1/32".to_string());
        explicit_edge_ip_on_single_host_cidr.edge_ip = Some("192.168.8.1".to_string());
        assert!(resolve_egress_nat_config(&explicit_edge_ip_on_single_host_cidr).is_ok());

        let mut edge_ip_outside_lan_cidr = egress_section(true);
        edge_ip_outside_lan_cidr.edge_ip = Some("192.168.9.1".to_string());
        assert!(resolve_egress_nat_config(&edge_ip_outside_lan_cidr).is_err());
    ...
    fn f3_hive_id_traversal_is_rejected() {
        ...
        assert!(!hive_dir_is_direct_child(Path::new("/etc/passwd"), root));
    }
    ...
    fn render_worker_egress_yaml_validates_gateway() {
        ...
        let yaml = render_worker_egress_yaml(Some(&section)).unwrap();
        assert!(yaml.contains("gateway_ip: \"192.168.8.1\""));
        assert!(yaml.contains("ipv6: \"blocked\""));

        section.gateway_ip = Some("not-an-ip".to_string());
        assert!(render_worker_egress_yaml(Some(&section)).is_err());

        section.gateway_ip = Some("192.168.8.1".to_string());
        section.ipv6 = "allowed".to_string();
        assert!(render_worker_egress_yaml(Some(&section)).is_err());
    }
    ...
    fn resolve_add_hive_ingress_section_requires_and_validates_listen() {
        // happy: explicit listen plus TLS vault key.
        let s = resolve_add_hive_ingress_section(&serde_json::json!({
            "ingress": { "listen": "0.0.0.0:443", "tls_vault_key": "edge_tls" }
        }))
        .unwrap();
        assert_eq!(s.listen.as_deref(), Some("0.0.0.0:443"));
        assert_eq!(s.tls_vault_key.as_deref(), Some("edge_tls"));
        ...
        // missing TLS is rejected unless plaintext is explicit.
        assert!(resolve_add_hive_ingress_section(&serde_json::json!({
            "ingress": { "listen": "0.0.0.0:8443" }
        }))
        .is_err());
        assert!(resolve_add_hive_ingress_section(&serde_json::json!({
            "ingress": { "listen": "127.0.0.1:8080", "allow_plaintext": true }
        }))
        .is_ok());
        assert!(resolve_add_hive_ingress_section(&serde_json::json!({
            "ingress": { "listen": "0.0.0.0:443", "allow_plaintext": true }
        }))
        .is_err());
        ...
        assert!(resolve_add_hive_ingress_section(&serde_json::json!({
            "ingress": {
                "listen": "0.0.0.0:443",
                "tls_vault_key": "edge_tls",
                "endpoints_json": { "endpoints": [{ "ich": "ich:seed" }] }
            }
        }))
        .is_err());
        assert!(resolve_add_hive_ingress_section(&serde_json::json!({
    ...
    #[test]
    fn sy_edge_unit_matches_ingress_dependencies() {
        let unit = core_service_unit_contents("sy-edge", "/usr/bin/sy-edge");
    ...
    fn resolve_add_hive_ssh_creds_requires_user_and_optional_password() {
        ...
        let creds = resolve_add_hive_ssh_creds(
            &serde_json::json!({"ssh_user": "admin", "ssh_password": "   "}),
        )
        .unwrap();
        assert!(creds.password.is_none());
    ...
    fn bootstrap_authorized_key_cleanup_script_is_user_level() {
        let public_key =
            "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIFakeKeyMaterial fluxbee-orchestrator";
    ...
    fn askpass_script_is_owner_only_and_round_trips_special_chars() {
        ...
        for pw in [
            ...
            assert!(
                out.status.success(),
                "askpass script must execute cleanly for {pw:?}"
            );
            assert_eq!(
  ...
  <folder id=\"{dist_id}\" label=\"Fluxbee Dist\" path=\"/var/lib/fluxbee/dist\" type=\"sendreceive\">
    ...
    fn f11_remove_hive_cleanup_appends_egress_teardown_only_for_egress() {
        ...
        let nft_at = egress
            .find("nft delete table")
            .expect("nft teardown present");
        let kill_at = egress
            .find("systemctl kill")
...
"#;
    ...
    #[test]
    fn runtime_spawn_preflight_with_manifest_includes_materialization_for_full_runtime() {
        let root = std::env::temp_dir().join(format!("fluxbee-spawn-preflight-{}", Uuid::new_v4()));
        let runtime = "ai.spawn.test";
        let version = "1.0.0";
        let manifest = RuntimeManifest {
            schema_version: 2,
            version: 1710000015555,
            updated_at: Some("2026-03-30T02:20:00Z".to_string()),
            runtimes: serde_json::json!({
                runtime: {
                    "available": [version],
                    "current": version,
        ...
        let out = runtime_spawn_preflight_with_manifest_and_root(
            &manifest,
            runtime,
            version,
            &entrypoint,
            "worker-220",
            "AI.spawn.test@worker-220",
            &root,
        )
        .expect_err("missing start.sh must fail");

        assert_eq!(out["error_code"], "RUNTIME_NOT_PRESENT");
        assert!(out.get("runtime_materialization").is_some());
        assert_eq!(
            out["runtime_materialization"]["versions"][version]["blocking_reason"],
            serde_json::json!("missing_package_dir")
        );
        assert_eq!(out["targeted_runtime_health"]["scope"], "targeted");
        assert_eq!(out["global_runtime_health"]["scope"], "global_current");
    ...
    #[test]
    fn runtime_spawn_preflight_with_manifest_includes_base_materialization_details() {
        let root = std::env::temp_dir().join(format!("fluxbee-spawn-base-{}", Uuid::new_v4()));
        let runtime = "wf.onboarding.standard";
        let version = "1.0.0";
        let base_runtime = "wf.engine";
        let base_version = "9.9.1";
        fs::create_dir_all(root.join(runtime).join(version)).expect("requested runtime dir");

        let manifest = RuntimeManifest {
            schema_version: 2,
            version: 1710000016666,
            updated_at: Some("2026-03-30T02:30:00Z".to_string()),
            runtimes: serde_json::json!({
                runtime: {
                    "available": [version],
                    "current": version,
        ...
        let out = runtime_spawn_preflight_with_manifest_and_root(
            ...
            &entrypoint,
            "worker-220",
            "WF.onboarding.standard@worker-220",
            &root,
        )
        .expect_err("missing base start.sh must fail");

        assert_eq!(out["error_code"], "BASE_RUNTIME_NOT_PRESENT");
        assert!(out.get("requested_runtime_materialization").is_some());
        assert!(out.get("runtime_base_materialization").is_some());
        assert_eq!(
            out["requested_runtime_materialization"]["versions"][version]["blocking_reason"],
            serde_json::json!("missing_base_start_script")
        );
        assert_eq!(
            out["runtime_base_materialization"]["versions"][base_version]["blocking_reason"],
            serde_json::json!("missing_package_dir")
        );
    ...
    #[test]
    fn runtime_health_snapshot_reports_targeted_ok_and_global_degraded() {
        let root = std::env::temp_dir().join(format!("fluxbee-health-scope-{}", Uuid::new_v4()));
        let target_runtime = "io.slack";
        let target_version = "1.1.0";
        let unrelated_runtime = "ai.common";
        let unrelated_version = "0.1.2";
        ...
        let manifest = RuntimeManifest {
            schema_version: 2,
            version: 1710000014444,
            updated_at: Some("2026-03-30T02:10:00Z".to_string()),
            runtimes: serde_json::json!({
                target_runtime: {
                    "available": [target_version],
                    "current": target_version,
        ...
        let targeted =
            runtime_health_snapshot(&manifest, &root, Some(target_runtime), Some(target_version))
                .expect("targeted health");
        assert_eq!(targeted["status"], "ok");
        assert_eq!(targeted["scope"], "targeted");

        let global =
            runtime_health_snapshot(&manifest, &root, None, None).expect("global runtime health");
        assert_eq!(global["status"], "degraded");
        assert_eq!(global["scope"], "global_current");
        assert!(
            global["errors"]
                .as_array()
                .expect("global errors array")
                .iter()
                .any(|value| value.as_str().unwrap_or("").contains("runtime='ai.common'")),
            "expected unrelated missing runtime in global health: {}",
            global
        );
    ...
    #[test]
    fn runtime_materialization_for_entry_reports_current_blocking_reason() {
        let root = std::env::temp_dir().join(format!("fluxbee-materialization-{}", Uuid::new_v4()));
        let runtime = "ai.common";
        let version = "0.1.2";
        let package_dir = root.join(runtime).join(version);
        fs::create_dir_all(&package_dir).expect("create package dir without start.sh");
        ...
        let detail =
            runtime_materialization_for_entry_with_root(runtime, &entry, None, &root, None);
        let current = detail["versions"][version].clone();
        assert_eq!(current["materialized"], serde_json::json!(false));
        assert_eq!(
